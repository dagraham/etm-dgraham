...
item_hsh:    {self.item_hsh}
    ...
    def reschedule(self, doc_id, old_dt, new_dt):
        """
        Reschedules a document with the given `doc_id` from the old date `old_dt` to the new date `new_dt`.

        Parameters:
            doc_id (str): The ID of the document to be rescheduled.
            old_dt (datetime): The old date of the document.
            new_dt (datetime): The new date of the document.

        Returns:
            bool: True if the rescheduling is successful, False otherwise.
        """
        changed = False
        self.doc_id = doc_id
        self.item_hsh = self.db.get(doc_id=doc_id)
    ...
    def do_quota(self, arg: str)->list[int]:
        ...
        if m:
            quota, period, periods_str = m.groups()
            logger.debug(f"{periods_str = }")
            period = 'w' if not period else period
            if periods_str and period != 'y':
                periods_str = periods_str.lstrip(':').strip()
                r = periods_regex.match(periods_str)
                if r:
                    ...
                    logger.debug(f"{quota = }; {period = }; {periods_str = }; {periods = }")

            rep = f'{quota} times/{period_dict[period]}{rep_periods}'
            obj = [quota, period, periods]
            self.item_hsh['q'] = obj
...
def quota_as_string(s):
    ...
    if len(s) > 2 and s[2]:
        logger.debug(f"{s = }")
        if len(s[2]) > 2 and [x for x in range(s[2][0], s[2][-1]+1)] == s[2]:
            tmp = f"{s[2][0]}-{s[2][-1]}"
        else:
            tmp = ','.join(f'{i}' for i in s[2])
        return f'{s[0]}{s[1]}: {tmp}' if tmp else f'{s[0]}{s[1]}'
    elif len(s) > 1:
        return f'{s[0]}{s[1]}'
    else:
        return f'{s[0]}w'
...
#     return dataview.current, dataview.alerts, dataview.id2relevant, False
...
def show_goals(
    ...
    for item in db:
            ...
            if quota < 0:
                ...
                if periods:
                    ...
                    if active_hsh[period] in periods:
                        ...
                        path = 'Inactive'
                        goal = f"{abs(quota)}{period}[{period_str}]"
                        itemtype = EtmChar.INACTIVE_CHAR
                else:
                    path = 'Active'
                    goal = f"{done}/{abs(quota)}{period}"
        ...
        rows.append(
            {
                'sort': sort,
                'path': active_str if path == 'Active' else path,
...
def create_item_views(item, flags):
    ...
    pinned_list = dataview.pinned_list
    ...
    for dt, et in instances:
        ...
        if 'j' in item:
            ...
            if after:
                rows.append(after)
    logger.debug(f"create_item_views: {views = }")
    return views
...
def schedule(
    ...
    """
    global current_hsh, active_tasks, aft_dt, bef_dt, width, omit, created_used, today, tomorrow, summary_width, todayYMD, tomorrowYMD, num_remaining
    dataview = Dataview()
    ...
    konnected = [x for x in dataview.konnections_to] + [
        x for x in dataview.konnections_from
    ]
    dataview.konnected = list(set(konnected))
    dataview.konnected.sort()
    row = 0
    for id in dataview.konnected:
        row += 1
        dataview.konnected_id2row[id] = row
    ...
    logger.debug(f'current: {current}')
    dataview.current = current
    dataview.alerts = alerts
