...
def on_ask_change(edit, new_edit_text):
    ...
    if at_tups:
        ...
        if itemtype in type_keys:
            ...
            if act_key == itemtype:
                if act_val:
                    reply.set_text(('I say', "{0} summary (string):\n  {1} {2}".format(type_keys[itemtype], act_key, act_val)))
                else:
                    reply.set_text(('I say', "{0} summary (string):\n  {1}".format(type_keys[itemtype], act_key)))

            elif act_key in at_keys:
                if act_val:
                    reply.set_text(('I say', "{0}:\n  @{1} {2}".format(at_keys[act_key], act_key, act_val)))
                else:
                    reply.set_text(('I say', "{0}:\n  @{1}".format(at_keys[act_key], act_key)))
            else:
