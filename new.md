...
- `$`: action
    ...
    - In *all* cases - events, tasks, journal entries as well as in actions themselves - timer information is recorded using the `@m`, *memo* entry. The format is `@m datetime started, timeperiod active,  datetime finished`. 
...
The *Agenda*, *Done* and *Busy* views use weekly diplays which are synchronized so that switching from one of these views to another always displays the same week. 
