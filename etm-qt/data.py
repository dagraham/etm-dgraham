...
## for those without integer prefixes
...
class Period:
    def __init__(self, datetime1, datetime2):
        ...
        if aware1:
            ...
            self.end = datetime2.astimezone(timezone('UTC'))
        else:
...
def encode_duration_list(obj_lst):
    try:
        return ", ".join([encode_duration(x) for x in obj_lst])
    except Exception as e:
        print('encode_duration_list', e)
        print(obj_lst)
...
def decode_duration(s):
    ...
    Examples:
        decode_duration('-2w3d4h5m')= Duration(weeks=-2,days=3,hours=4,minutes=5)
        decode_duration('1h30m') = Duration(hours=1, minutes=30)
        decode_duration('-10m') = Duration(minutes=10)
    where:
        w: weeks
        d: days
        h: hours
    ...
    >>> 3*60*60+5*60
    11100
    >>> decode_duration("2d-3h5m")[1]
    Duration(days=1, hours=21, minutes=5)
    >>> datetime(2015, 10, 15, 9, 0, tz='local') + decode_duration("-25m")[1]
    DateTime(2015, 10, 15, 8, 35, 0, tzinfo=Timezone('America/New_York'))
    >>> datetime(2015, 10, 15, 9, 0) + decode_duration("1d")[1]
    DateTime(2015, 10, 16, 9, 0, 0, tzinfo=Timezone('UTC'))
    >>> datetime(2015, 10, 15, 9, 0) + decode_duration("1w-2d+3h")[1]
